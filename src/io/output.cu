...
namespace io
{
    ...
    static void write_annul_pressure(const LBMState &S, int t, const std::string &out_dir, const DomainTags &tags)
    {
        ...
        for (int x = x_begin; x < x_end; ++x)
        {
            ...
            const real_t rho_p = (S.h_rho[idx] + S.h_rho[idx_n]) * r::half;
    ...
    static void write_annul_profile(const LBMState &S, int t, const std::string &out_dir, const DomainTags &tags)
    {
        ...
        for (int x = x_begin; x < x_end; ++x)
        {
            ...
            const real_t ux = (S.h_ux[idx] / Stencil::as2 + S.h_ux[idx_n] / Stencil::as2) * r::half;
            const real_t uy = (S.h_uy[idx] / Stencil::as2 + S.h_uy[idx_n] / Stencil::as2) * r::half;
    ...
    {
        write_annul_profile(state, t, out_dir, tags);
        write_annul_pressure(state, t, out_dir, tags);
    }
}